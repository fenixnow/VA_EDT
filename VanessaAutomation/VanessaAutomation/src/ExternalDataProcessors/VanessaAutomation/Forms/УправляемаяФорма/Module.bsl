...
Процедура ВыгрузитьНастройкиВФайлЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	НаДругомЯзыке = Ложь;
	НаименованиеКлючейНаРазныхЯзыках = Неопределено;
	Если Объект.ЯзыкГенератораGherkin <> "ru" Тогда
		НаДругомЯзыке = Истина;
		ТекущийЯзык = Объект.ЯзыкГенератораGherkin;
		НаименованиеКлючейНаРазныхЯзыках = Локализация(ТекущийЯзык);		
	КонецЕсли;
	...
	Для каждого СтрокаНастроек Из Настройки Цикл
		...
		ИмяКлючаСвойства = СтрокаНастроек.Ключ;
		
		Если НаДругомЯзыке Тогда
			ИмяЛокализованногоКлюча = РезультатЛокализации(НаименованиеКлючейНаРазныхЯзыках, ИмяКлючаСвойства);
			Если ИмяЛокализованногоКлюча <> Неопределено Тогда
				ИмяКлючаСвойства = ИмяЛокализованногоКлюча;
			КонецЕсли;
		КонецЕсли;
		...
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("СписокЗначений") Тогда 
			Если СтрокаНастроек.Значение.Количество() > 0 Тогда 
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлючаСвойства);
				ЗаписьJSON.ЗаписатьНачалоМассива();
		...
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("Структура") Тогда
			...
			Если ИмяКлючаСвойства = "ВыполнениеСценариев" 
				Или ИмяКлючаСвойства = "КлиентТестирования" Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлючаСвойства);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
					...
					Если НаДругомЯзыке Тогда
						ИмяЛокализованногоКлючаСтруктуры = РезультатЛокализации(НаименованиеКлючейНаРазныхЯзыках, ИмяКлючаСтруктуры);
						Если ИмяЛокализованногоКлючаСтруктуры <> Неопределено Тогда
							ИмяКлючаСтруктуры = ИмяЛокализованногоКлючаСтруктуры;
						КонецЕсли;
					КонецЕсли;
			...
			КонецЕсли;
			// Отчет Allure
			Если ИмяКлючаСвойства = "ОтчетAllure" И Настройки.ДелатьОтчетВФорматеАллюр Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлючаСвойства);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
					ИмяКлючаСтруктуры = СтрокаСтруктуры.Ключ;
			...
			КонецЕсли;
			// Отчет JUnit
			Если ИмяКлючаСвойства = "ОтчетJUnit" И Настройки.ДелатьОтчетВФорматеjUnit Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлючаСвойства);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
			...
			КонецЕсли;
			// Отчет Cucumber
			Если ИмяКлючаСвойства = "ОтчетCucumber" И Настройки.ДелатьОтчетВФорматеCucumberJson Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлючаСвойства);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаСтруктуры.Значение Цикл
					ИмяКлючаСтруктуры = СтрокаСтруктуры.Ключ;
			...
			КонецЕсли;
			// Инструкция HTML
			Если ИмяКлючаСвойства = "ИнструкцияHTML" И Настройки.СоздаватьИнструкциюHTML Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлючаСвойства);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаСтруктуры.Значение Цикл
					ИмяКлючаСтруктуры = СтрокаСтруктуры.Ключ;
			...
			КонецЕсли;
			// Инструкция Markdown
			Если ИмяКлючаСвойства = "ИнструкцияMarkdown" И Настройки.СоздаватьИнструкциюMarkdown Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлючаСвойства);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаСтруктуры.Значение Цикл
					ИмяКлючаСтруктуры = СтрокаСтруктуры.Ключ;
			...
			КонецЕсли;
			// Запись видео
			Если ИмяКлючаСвойства = "ЗаписьВидео" И Настройки.СоздаватьИнструкциюВидео Тогда
			...
			КонецЕсли;
			// SikuliXServer
			Если ИмяКлючаСвойства = "SikuliXServer" И Настройки.ИспользоватьSikuliXСервер Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлючаСвойства);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
					ИмяКлючаСтруктуры = СтрокаСтруктуры.Ключ;
