...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли Найти(ИмяСобытия, "ВыполнитьСценарий") > 0
			Или Найти(ИмяСобытия, "ЗагрузитьСценарий") Тогда
...
Функция ПолучитьПутьКFeatureReader() Экспорт
	ПутьКФайлу = ДополнитьСлешВПуть(Объект.КаталогИнструментов) + "lib/FeatureReader/FeatureReader.epf";
...
Функция ИменаКлючейОбщихНастроек()
	...
	ТаблицаИмен.Колонки.Добавить("ИмяРусское");
	ТаблицаИмен.Колонки.Добавить("ИмяАнглийское");
...
&НаСервереБезКонтекста
Процедура ТаблицаИменНоваяСтрока(ИмяРусское, ИмяАнглийское, Таб)

	НоваяСтрока = Таб.Добавить();
	НоваяСтрока.ИмяРусское = ИмяРусское;
	НоваяСтрока.ИмяАнглийское = ИмяАнглийское;
...
//		ОсновныеНастройки = ТекущиеОбщиеНастройки(, Истина);
...
СтрокаНастроек = ВыгрузитьОбщиенастройкиНаСервере(Настройки, ТекущийЯзык);
	
	ЗаписатьОбщиеНастройкиВФайл(ПолноеИмяФайла, СтрокаНастроек, ТекущийЯзык);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ВыгрузитьОбщиенастройкиНаСервере(Настройки, ТекущийЯзык)
	
		НаДругомЯзыке = Ложь;
	СоответствиеИменКлючей = Неопределено;
	...
	Для каждого СтрокаНастроек Из Настройки Цикл
		...
		Если НаДругомЯзыке Тогда
			СтрокаСИменемКлюча = ТаблицаИменКлючей.Найти(ИмяКлюча, "ИмяРусское");
			Если СтрокаСИменемКлюча <> Неопределено Тогда
				ИмяКлюча = СтрокаСИменемКлюча.ИмяАнглийское;
			КонецЕсли;
		КонецЕсли;
		...
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("Структура") Тогда
			...
			Если ИмяКлюча = "ВыполнениеСценариев" 
				Или ИмяКлюча = "КлиентТестирования" Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлюча);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
					...
					Если НаДругомЯзыке Тогда
						СтрокаСИменемКлюча = ТаблицаИменКлючей.Найти(ИмяКлючаСтруктуры, "ИмяРусское");
						Если СтрокаСИменемКлюча <> Неопределено Тогда
							ИмяКлючаСтруктуры = СтрокаСИменемКлюча.ИмяАнглийское;
						КонецЕсли;
					КонецЕсли;
			...
			КонецЕсли;
			// Отчет Allure
			Если ИмяКлюча = "ОтчетAllure" И Настройки.ДелатьОтчетВФорматеАллюр Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлюча);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
					ИмяКлючаСтруктуры = СтрокаСтруктуры.Ключ;
			...
			КонецЕсли;
			// Отчет JUnit
			Если ИмяКлюча = "ОтчетJUnit" И Настройки.ДелатьОтчетВФорматеjUnit Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлюча);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
			...
			КонецЕсли;
			// Отчет Cucumber
			Если ИмяКлюча = "ОтчетCucumber" И Настройки.ДелатьОтчетВФорматеCucumberJson Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлюча);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаСтруктуры.Значение Цикл
					ИмяКлючаСтруктуры = СтрокаСтруктуры.Ключ;
			...
			КонецЕсли;
			// Инструкция HTML
			Если ИмяКлюча = "ИнструкцияHTML" И Настройки.СоздаватьИнструкциюHTML Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлюча);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаСтруктуры.Значение Цикл
					ИмяКлючаСтруктуры = СтрокаСтруктуры.Ключ;
			...
			КонецЕсли;
			// Инструкция Markdown
			Если ИмяКлюча = "ИнструкцияMarkdown" И Настройки.СоздаватьИнструкциюMarkdown Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлюча);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаСтруктуры.Значение Цикл
					ИмяКлючаСтруктуры = СтрокаСтруктуры.Ключ;
			...
			КонецЕсли;
			// Запись видео
			Если ИмяКлюча = "ЗаписьВидео" И Настройки.СоздаватьИнструкциюВидео Тогда
			...
			КонецЕсли;
			// SikuliXServer
			Если ИмяКлюча = "SikuliXServer" И Настройки.ИспользоватьSikuliXСервер Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(ИмяКлюча);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
					ИмяКлючаСтруктуры = СтрокаСтруктуры.Ключ;
...
&НаКлиенте
Процедура ЗаписатьОбщиеНастройкиВФайл(ПолноеИмяФайла, СтрокаНастроек, ТекущийЯзык)
	
	Если ПустаяСтрока(ТекущийЯзык) Тогда
		ТекущийЯзык  = "ru";
	КонецЕсли;
	
	ВерсияОбработки = СтрЗаменить(КешВерсияОбработки, "ver ", "");
	ТекстЗаголовка = "### VA params file. version=""%1"" lang=""%2"" ###";
	ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", ВерсияОбработки);
	ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%2", ТекущийЯзык);
	
	ЗаписьТекста = Новый ЗаписьТекста(ПолноеИмяФайла, КодировкаТекста.UTF8);
	ЗаписьТекста.ЗаписатьСтроку(ТекстЗаголовка);
...
//	Если НастройкиИзФайла.ТекущийЯзык <> "ru" Тогда
//		СоответствиеИменКлючей = СоответствиеИменКлючейСАнглийскимЯзыком();
//		НастройкиИзФайла.Вставить("СоответствиеИменКлючей", СоответствиеИменКлючей);
//	КонецЕсли;
...
&НаСервере
Процедура ЗагрузитьНастройкиНаСревере(СтруктураНастроек)
	
	Если СтруктураНастроек.Свойство("НовыйФорматИзФайла") Тогда
		ВерсияОбработки = СтруктураНастроек.ВерсияОбработки;
		ТекущийЯзык = СтруктураНастроек.ТекущийЯзык;
		Настройки = СтруктураНастроек.Настройки;
	Иначе
		ВерсияОбработки = "";
		ТекущийЯзык = "ru";
...
//	КонецЕсли;
	...
	Для каждого СтрокаНастроек Из Настройки Цикл
		...
		Если НаДругомЯзыке Тогда
			СтрокаСИменемКлюча = ТаблицаИменКлючей.Найти(СтрокаНастроек.Ключ, "ИмяАнглийское");
			Если СтрокаСИменемКлюча <> Неопределено Тогда
				ИмяКлюча = СтрокаСИменемКлюча.ИмяРусское;
			КонецЕсли;
		КонецЕсли;
...
//				СообщитьПользователю(СтрокаОшибки);
		...
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("Структура") Тогда
			Для Каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл 
				Если ТипЗнч(СтрокаСтруктуры.Значение) = Тип("Массив") Тогда
					// Данные меток Allure
					Если ИмяКлюча = "ДанныеАллюрМеток" Тогда
				...
				КонецЕсли;
				Попытка
					Объект[ИмяКлюча] = СтрокаСтруктуры.Значение;
				Исключение
//					СтрокаОшибки = ЛокализоватьСервер("Параметр: %1 не загружен.");
...
&НаКлиенте
Функция СтруктураЗагрузкиОбщихНастроек()

	СтруктураЗагрузки = Новый Структура;
	СтруктураЗагрузки.Вставить("ВерсияОбработки", "");
	СтруктураЗагрузки.Вставить("ТекущийЯзык", "");
	СтруктураЗагрузки.Вставить("Настройки");	
	СтруктураЗагрузки.Вставить("НовыйФорматИзФайла", Ложь);
	
	Возврат СтруктураЗагрузки;
...
&НаКлиенте
Функция Локализация(ТекущийЯзык, ОбъектЛокализации = "")

	ИмяФайлаЛокализацииПараметров = ОбъединитьПути(ПолучитьПутьКОбработкеКлиент(), "locales", ТекущийЯзык, , "params.json");

	ФайлЛокализации = Новый Файл(ИмяФайлаЛокализацииПараметров);

	Если Не ФайлЛокализации.Существует() Тогда
		Возврат Неопределено;
	КонецЕсли;

	ЧтениеJSON = Новый ЧтениеJSON;
	ЧтениеJSON.ОткрытьФайл(ИмяФайлаЛокализацииПараметров);

	Попытка
		ДанныеЛокализации = ПрочитатьJSON(ЧтениеJSON);
		ЧтениеJSON.Закрыть();
...
&НаКлиенте
Функция РезультатЛокализации(ДанныеЛокализации, Имя)

	Если ДанныеЛокализации = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;

	Если ДанныеЛокализации.Свойство(Имя) Тогда
		Результат = ДанныеЛокализации[Имя];
