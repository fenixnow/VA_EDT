...
Функция ШаблонСценарияОткрытьФорму(Параметры)
	...
	Если Параметры.Тип <> "Обработка" Или Параметры.Тип <> "Отчет" Тогда
		...
		Сценарий = Сценарий + "	
			...
			|	Тогда открылась форма с именем ""%ПолноеИмя%.ФормаСписка""
			|#	И Пауза 2
	...
	Сценарий = Сценарий + "
		...
		//FIXME: если у элемента нет формы, тогда объекта, иначе элемента
		|	Тогда открылась форма с именем ""%ПолноеИмя%.ФормаОбъекта""
		|#	И Пауза 2
		|	И Я закрываю текущее окно
		|";
