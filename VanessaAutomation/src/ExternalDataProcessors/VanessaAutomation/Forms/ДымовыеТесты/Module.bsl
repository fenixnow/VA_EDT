...
Процедура ПодготовитьДанныеДерева()
	...
	Тексты = СформироватьТекстыСценариев(МассивСценариев);

ТекДокумент = Новый ТекстовыйДокумент();
ТекДокумент.ДобавитьСтроку(Тексты);
...
&НаСервере
Функция СформироватьТекстыСценариев(МассивСценариев)
	
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("features");
	...
	Для Каждого СтрокаСценария Из МассивСценариев Цикл
		...
		Если СтрокаСценария.СценарийОткрытьФорму Тогда
			...
			ЗаписьТекста.ЗаписатьСтроку(ТекстСценария);
			ЗаписьТекста.ЗаписатьСтроку("СценарийОткрытьФорму");	
		КонецЕсли;
		
		Если СтрокаСценария.СценарийЗаписьНового Тогда
			ЗаписьТекста.ЗаписатьСтроку(СтрокаСценария.ПолноеИмя);
			ЗаписьТекста.ЗаписатьСтроку("СценарийЗаписьНового");
		КонецЕсли;
	
		Если СтрокаСценария.СценарийЗаписьСуществующего Тогда
			ЗаписьТекста.ЗаписатьСтроку(СтрокаСценария.ПолноеИмя);
			ЗаписьТекста.ЗаписатьСтроку("СценарийЗаписьСуществующего");
		КонецЕсли;
	
		Если СтрокаСценария.СценарийВводНаОсновании Тогда
			ЗаписьТекста.ЗаписатьСтроку(СтрокаСценария.ПолноеИмя);
			ЗаписьТекста.ЗаписатьСтроку("СценарийВводНаОсновании");
		КонецЕсли;
	...
	тексты = текстовыйДокумент.ПолучитьТекст();
...
Функция ШаблонСценарияОткрытьФорму(Параметры)
	...
	Текст = "#language: ru
		...
		|@tree
		|
		|Функциональность: Проверка открытия форм списка
		|
		|Контекст:
		...
		|		
		|	Дано Я открываю навигационную ссылку ""e1cib/list/%ПолноеИмяОбъекта%""
		|
		|";
	
	Текст = СтрЗаменить(Текст, "%ПолноеИмяОбъекта%", Параметры.Имя);	
