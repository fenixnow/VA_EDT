...
Процедура СформироватьТекстыСценариев(МассивСценариев)
	...
	Для Каждого СтрокаСценария Из МассивСценариев Цикл
		...
		Если СтрокаСценария.СценарийОткрытиеФормы Тогда
			ТекстСценария = ШаблонСценарияОткрытьФорму(СтрокаСценария);
			
			ЗаписьТекста.ЗаписатьСтроку(ТекстСценария);
			ЗаписьТекста.ЗаписатьСтроку("СценарийОткрытиеФормы");	
		КонецЕсли;
...
Функция СтруктураСценария()
	...
	СтруктураСценария.Вставить("СценарийОткрытиеФормы", Ложь);
...
Функция ШаблонСценарияОткрытьФорму(Параметры)
	...
	Текст = СтрЗаменить(Текст, "%ПолноеИмяОбъекта%", Параметры.ИмяОбъекта);	
