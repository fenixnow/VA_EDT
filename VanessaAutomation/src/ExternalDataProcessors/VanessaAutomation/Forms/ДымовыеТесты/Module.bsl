...
&НаКлиенте
Процедура СгенерироватьСценарии(Команда)
...
Процедура СформироватьТекстыСценариев(МассивСценариев)
	...
	Для Каждого СтрокаСценария Из МассивСценариев Цикл
		...
		Если СтрокаСценария.СценарийОткрытьФорму Тогда
			ТекстСценария = ШаблонСценарияОткрытьФормуСписка(СтрокаСценария);
...
&НаСервере
Функция ШаблонСценарияОткрытьФормуСписка(Параметры)

	Текст = "	
		|Сценарий: Открытие формы списка ""%Имя%"" 
		|		
		|	Дано Я открываю навигационную ссылку ""e1cib/list/%ПолноеИмя%""
		|	Тогда открылось окно ""*""
		|	И Пауза 2
		|	И я закрываю текущее окно";

	Текст = СтрЗаменить(Текст, "%ПолноеИмя%", Параметры.ПолноеИмя);
	Текст = СтрЗаменить(Текст, "%Имя%", Параметры.Имя);
...
Процедура ПодготовитьСценарииКВыполнению(Команда)
	...
	ИмяВременногоФайла = КаталогФайлов + ПолучитьРазделительПути()
		+ "temp.feature";
