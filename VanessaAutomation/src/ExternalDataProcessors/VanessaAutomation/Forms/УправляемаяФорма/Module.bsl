...
Процедура ПодготовитьСозданиеИнструкцииHTML()
	...
	Если НЕ ФайлСуществуетКомандаСистемы(КаталогВыгрузкиИнструкцийHTML) Тогда 
		ТекстОшибки = Локализовать("Файл КаталогВыгрузкиИнструкцияHTML: %1 не существует.");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", КаталогВыгрузкиИнструкцийHTML);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	 
...
Функция ПрочитатьЗаголовокФичи(ИмяФайла)
	...
	Пока Истина Цикл
		...
		КонецЕсли;	 
		Если Лев(Стр, 1) = "#" Тогда
		// TODO: дать возможность записывать строку или возвращать пустой массив
//			Если СтрНайти(Стр, "autodoc.ignore.header") > 0 Тогда
//				Массив.Добавить("[autodoc.ignore.header]");
//			КонецЕсли;
			Продолжить;
		КонецЕсли;	 
...
Процедура ПроставитьСвойстваШагаСУчетомСвойствГруппы(Шаг,СтрокаДереваШаг,Сценарий)
		...
		ИначеЕсли РодительШага.Тип = "Область" Тогда
			Если ТипЗнч(РодительШага.ПроизвольныеЗначения) = Тип("Структура") Тогда
				Для Каждого Комментарий Из РодительШага.ПроизвольныеЗначения.МассивКомментариевШага Цикл
					...
					ИначеЕсли СтрНайти(НРег(Комментарий), "autodoc.ignore.stepslide") > 0 Тогда
						Шаг.Вставить("ИгнорироватьСкриншот", Истина);
	...
	Шаг.Вставить("ИгнорироватьШагВАвтоинструкции",ИгнорироватьШаг);
КонецПроцедуры 
...
Процедура ОбработатьСлужебныеДирективыАвтоинструкций(Шаг,Сценарий)
	...
	Для Каждого КомментарийШага Из Шаг.МассивКомментариевШага Цикл
		...
		Поз = Найти(НРег(КомментарийШага),Директива); 
		Если Поз > 0 Или СтрНайти(НРег(КомментарийШага), "autodoc.ignore.step") > 0 Тогда
//			СлужебнаяСтрока = СокрЛП(Сред(КомментарийШага,Поз + СтрДлина(Директива)));
			Шаг.Вставить("ИгнорироватьШагВАвтоинструкции",Истина);
		КонецЕсли;
		
		Если СтрНайти(НРег(КомментарийШага), "autodoc.ignore.slide") > 0 Тогда
			Шаг.Вставить("ИгнорироватьСкриншот", Истина);
		КонецЕсли;
...
Процедура СоздатьФайлHTMLИнструкции()
	...
	CSSСтили = АвтоДокументация.CSSСтили();
	...
	Если ЗаголовокФичи.Количество() > 0 Тогда
		Для каждого Стр Из ЗаголовокФичи Цикл
	...
	Для каждого Сценарий Из МассивСценариев Цикл
		...
		Для каждого Шаг Из МассивШагов Цикл
			...
			Если Не Шаг.Свойство("ИгнорироватьКомментарий") Тогда
				Для Ккк = 0 По МассивСтрок.Количество() - 1 Цикл
					Стр = МассивСтрок[Ккк];
					Если Ккк = 0 Тогда
				...
				КонецЦикла;
			КонецЕсли;
			Если Не Шаг.Свойство("ИгнорироватьСкриншот") Тогда
...
Процедура ПодключитьМодульАвтоДокументации()
	...
	СлужебныеПеременные.Вставить("Ванесса", ЭтаФорма);
