...
&НаКлиенте
Процедура ОткрытьФормуДымовыхТестов(Команда)

	ОткрытьФорму(ПолучитьИмяОбработкиVA() + ".Форма.ДымовыеТесты",, ЭтаФорма);
		...
		ИначеЕсли СтрСтроки.Тип = "ШагСценарий" Тогда
			...
			Если СтрСтроки.ДопТип = "СтруктураСценария" Тогда //это вложенная структура сценария
				...
				Если СтрокаПримеров = Неопределено Тогда
					ТекстСообщения = ТекстСообщенияПользователюВызовСервера("Не найдена строка примеров во вложенной структуре сценария. <%1>",ДопПараметры.ТекстСообщенийПользователю);
