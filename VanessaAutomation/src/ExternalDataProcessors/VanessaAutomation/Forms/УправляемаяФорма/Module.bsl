...
Процедура ВыполнитьСценарии(ТекСценарий = Неопределено, ДопПараметры = Неопределено) Экспорт
	...
	Если Объект.СоздаватьИнструкциюВидео Тогда
		АвтоДокументация.ПрочитатьИзФичиПараметрыСборкиВидео(МассивСценариевДляВыполнения);
	КонецЕсли;	 
...
// Нужна, т.к. не работает вызов УправляемаяФорма.РазложитьСтрокуВМассивПодстрок() из других обработок.
&НаКлиенте
...
Функция ПолучитьСтрокуВычисливВнутренниеВыражения(Знач Текст) Экспорт
	...
	Текст = СтрЗаменить(Текст,"УправляемаяФорма.","ЭтаФорма.");
...
Процедура СделатьДействияПриЗаписиHTML(ПарамСтр = "") Экспорт
	...
	Если Объект.СоздаватьИнструкциюHTML Тогда
		...
		АвтоДокументация.ДобавитьВЛогИнструкцииHTMLВыполненияШага(,ТекИД_СценарияВМассиве, ТекИД_ШагаВМассиве,  СкриншотСделанВСценарии);
	...
	Если Объект.СоздаватьИнструкциюMarkdown Тогда
		...
		АвтоДокументация.ДобавитьВЛогИнструкцииMarkdownВыполненияШага(, СкриншотСделанВСценарии);
		СкриншотСделанВСценарии = Истина;
		УбратьПодсветкуАктвиногоЭлемента();
	КонецЕсли;	 
КонецПроцедуры
...
Процедура ПодключитьБиблиотекуРаботыСВидео()
	...
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("УправляемаяФорма",ЭтаФорма);
...
Процедура ВыполнитьШагПродолжение(РезультатПрохожденияШага,ОбработкаТеста)
	...
	Попытка
		...
		Если НЕ ТекШаг.ЭтоШагКонтекста Тогда
			АвтоДокументация.ДобавитьВЛогИнструкцииHTMLВыполненияШага(МассивСценариевДляВыполнения, ТекИД_СценарияВМассиве, ТекИД_ШагаВМассиве, СкриншотСделанВСценарии);
			АвтоДокументация.ДобавитьВЛогИнструкцииMarkdownВыполненияШага(МассивСценариевДляВыполнения, СкриншотСделанВСценарии);
		КонецЕсли;	 
...
Функция ПолучитьПараметрыЗаписиВидео()
	...
	ПараметрыВидео.Вставить("УправляемаяФорма", ЭтаФорма);
...
Процедура ВыполнитьСценарийАссинхронноТаймер()
	...
	Если ПрекратитьВыполнениеВсехСценариев ИЛИ ((МассивСценариевДляВыполнения.Количество() - 1) < ТекИД_СценарияВМассиве) Тогда
        ...
        ПутьКОбработке = ПолучитьПутьКОбработкеКлиент();
		АвтоДокументация.СоздатьФайлHTMLИнструкции();
		АвтоДокументация.СоздатьФайлMarkdownИнструкции();
	...
	АвтоДокументация.ПодготовитьСозданиеИнструкцииHTML(МассивСценариевДляВыполнения, ТекИД_СценарияВМассиве);
	АвтоДокументация.ПодготовитьСозданиеИнструкцииMarkdown(МассивСценариевДляВыполнения, ТекИД_СценарияВМассиве);
	...
	АвтоДокументация.ДобавитьВЛогИнструкцииHTMLНачалоСценария(МассивСценариевДляВыполнения, ТекИД_СценарияВМассиве);
	АвтоДокументация.ДобавитьВЛогИнструкцииMarkdownНачалоСценария(МассивСценариевДляВыполнения, ТекИД_СценарияВМассиве);
...
Процедура ПолучитьТекстФичиИзДействийПользователя(Результат)
	...
	Для НомерСтроки = 1 По ТекстРезультат.КоличествоСтрок() Цикл
		...
		ТекстРезультатКод.ДобавитьСтроку("УправляемаяФорма.Шаг(""" + СтрокаТекста + """);");
	КонецЦикла;
...
&НаКлиенте
Процедура ПодключитьМодульАвтоДокументации()

	АвтоДокументация = ПолучитьФорму(ПолучитьИмяОбработкиVA() + ".Форма.АвтоДокументация");

	СлужебныеПеременные = Новый Структура;
	СлужебныеПеременные.Вставить("УправляемаяФорма", ЭтаФорма);
	МодульРаботыСВидео.УстановитьСлужебныеПеременные(СлужебныеПеременные);
...
Процедура ОбработчикПодключенияБиблиотекПриОткрытии(Знач ПомещенныеФайлы, Знач ДополнительныеПараметры) Экспорт
	...
	ПодключитьМодульАвтоДокументации();
