...
Процедура ВыполнитьСценарии(ТекСценарий = Неопределено, ДопПараметры = Неопределено) Экспорт
	...
	Если Объект.СоздаватьИнструкциюВидео Тогда
		АвтоДокументация.ПрочитатьИзФичиПараметрыСборкиВидео(МассивСценариевДляВыполнения, МассивСтрокДереваДанныеФормы);
	КонецЕсли;	 
...
Процедура ВыполнитьСценарийАссинхронноТаймер()
	...
	АвтоДокументация.ПодготовитьСозданиеИнструкцииHTML(Объект, МассивСценариевДляВыполнения, ТекИД_СценарияВМассиве);
...
Процедура ПодключитьМодульАвтоДокументации()
	...
	АвтоДокументация.УстановитьСлужебныеПеременные(СлужебныеПеременные);
